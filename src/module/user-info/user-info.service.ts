...
export class UserInfoService {
  ...
  async getUserInformation(condition: Object): Promise<UserInformation>{
    const user = await this.userInfoRepository.findOne(condition);
  ...
  async updateUserInformation(id:number, modifiedUserInfo: UserInfoDto): Promise<UserInformation>{
    const user = await this.userInfoRepository.findOne({id});
    if(!user)throw new CustomException(USER_INFORMATION_NOT_FOUND);
